...
checkContains("Footer shared on all steps (shown when not hidden)", css, '.wizard-footer:not([hidden])');
checkNotContains("Upload no longer force-hides footer", css, 'body[data-step="upload"] .wizard-footer {\n  display: none');
checkContains("Upload in-card submit button hidden (action lives in footer)", css, '.upload-form-card .form-actions .submit-btn');
checkContains("Upload uses shared footer (Next drives handleSubmit)", appJs, "if (!nextBtn.disabled) handleSubmit()");
checkContains("Upload readiness gated on selected source", appJs, "function _canUpload");
checkContains("Upload Back hidden via visibility (keeps CTA right-aligned)", appJs, 'backBtn.style.visibility = "hidden"');
...
checkContains("Footer disabled (not hidden) when blocked", appJs, "nextBtn.disabled  = !canProceed");
